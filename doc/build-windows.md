...
Below are some notes on how to build Asynx for Windows.

The options known to work for building Asynx on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Asynx Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Asynx applications.
...
Note that for WSL the Asynx source path MUST be somewhere in the default mount file system, for
example /usr/src/asynx, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

Acquire the source in the usual way:

    git clone https://github.com/Theoretix/Asynx.git
...
way. This will install to `c:\workspace\asynx`, for example:

    make install DESTDIR=/mnt/c/workspace/asynx
